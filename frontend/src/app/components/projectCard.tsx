...
const ProjectCard: React.FC<{ inputProps: props }> = ({ inputProps }) => {
  ...
  const { id, name, currentFunding, reqFunding, description, github, video, isInvestor, handleFunding } = inputProps;
  return (
    <>
      <Card className="p-8 max-w-md">
        ...
        {isInvestor == true ? (
          <div className="flex space-x-4 justify-center">
            <Button className="bg-orange-600 mt-4 px-8 max-w-md" onPress={onOpen}>
              View Details
            </Button>
            <Button className="bg-orange-600 mt-4 px-8 max-w-md" onPress={onOpen2}>
              Start a poll
            </Button>
          </div>
        ) : (
              ...
              <>
                ...
                <ModalBody>
                  ...
                  {isInvestor && (
                    <Button
                      variant="flat"
                      className="font-semibold w-auto mt-2 hover:bg-orange-500"
                      onClick={() => {
                        handleFunding(id, reqFunding)
                      }}
                    >
                      Fund this idea 💸
                    </Button>
        ...
        <form action="/pollCreate" method="POST">
          <Modal
            backdrop="blur"
            className="dark text-orange-100"
            isOpen={isOpen2}
          ...
          >
            <ModalContent>
              {(onClose) => (
                <>
                  <ModalHeader className="flex flex-col gap-1">Add a Poll</ModalHeader>
                  <ModalBody>
                    <Input
                      label="Prompt"
                      variant="underlined"
                      defaultValue={"Shall I continue funding?"}
                    />
                    <RadioGroup color="warning" label="Your Choice: ">
                  ...
                  <ModalFooter>
                    <Button className="bg-orange-600" onPress={onClose} type="submit">
                      Add Poll
                    </Button>
                  </ModalFooter>
                </>
